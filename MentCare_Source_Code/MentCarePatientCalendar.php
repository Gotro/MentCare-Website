...
class MentCarePatientCalendar {
	...
	public function __construct($aServer, $aUser, $aPassword, $aDatabase, $aPatientID) {
	    $server = $aServer;
		$user = $aUser;
		$password = $aPassword;
		$database = $aDatabase;
	...
	private function _showDay($cellNumber){
        ...
        if( ($this->currentDay!=0)&&($this->currentDay<=$this->daysInMonth) ){
			...
			$appointmentQuery = "select * from appointments left join patient
	                            ...
	                            on patient.userID = users.userID where users.userID = ".$this->userID." && date = '".$curDate."'";
			
			
			$appointmentResults = mysqli_query($this->connect, $appointmentQuery);
			$things = mysqli_fetch_assoc($appointmentResults);
			
            $cellContent = $this->currentDay."<br> Appointments: <br>Time: ".$things['time'];
